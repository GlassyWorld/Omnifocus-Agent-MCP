...
强制，而不是客户端 allowlist。`personal-production` 当前公开四个 Domain read tools 和
...
自动写入不是个性化架构的一部分。当前个性化 workflow 不会根据 Snapshot 或 AI 分析自动创建、编辑、完成或删除 OmniFocus 对象；`create_task` 仅响应用户新的、明确的单个 Inbox Task 创建请求。
...
前四个是稳定 Domain read tools；`create_task` 是唯一受 feature flag、显式授权和幂等保护约束的 mutation。该 Profile 不注册 Resources。
...
`get_project`、`get_task`、`get_completed_since` 和 `create_task`，不注册 generic read tools、
其他 mutation tools 或 MCP Resources。因此当前能力边界不依赖客户端 Tool allowlist。
`create_task` 只允许显式创建单个 Inbox Task；不能从 Profile 名称或该 Tool 推断、自动获得
任何其他写入能力。
