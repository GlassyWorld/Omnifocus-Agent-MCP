...
`personal-production` 是面向日常 ChatGPT App 的精选生产能力集合。当前版本只暴露和调用读取能力，核心接口为：
...
tools 或 MCP Resources。因此当前能力边界不依赖客户端 Tool allowlist。未来如需写入，必须
通过专门设计、显式注册并具备授权与确认约束的受控 Tool 加入，不能从 Profile 名称推断或
自动获得写入能力。
