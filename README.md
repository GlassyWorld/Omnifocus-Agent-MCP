...
完整 path 消歧。该 Tool 不创建或修改 Tag，也不构成 `create_task` Tag 写入授权。
...
自动写入不是个性化架构的一部分。当前个性化 workflow 不会根据 Snapshot 或 AI 分析自动创建、编辑、完成或删除 OmniFocus 对象；`create_task` V2 仅响应用户新的、明确的单个 Inbox 或 exact Active Project Task 创建请求，且不支持 Tag assignment。
...
`create_task` V2 只允许显式创建单个 Inbox 或 exact Active Project Task；不能从 Profile 名称或该 Tool 推断、自动获得
任何其他写入能力。
