...
export const TOOL_REGISTRY: readonly ToolRegistration[] = [
  ...
  {
    name: "create_task",
    description: "Create exactly one new OmniFocus task after an explicit user request. destination is required and must be either Inbox or one exact Active Project canonical ID returned by a fresh get_project read. Optional tagIds accepts 1-5 unique canonical IDs of existing Active Tags selected from fresh search_tags results; names and paths are never mutation identifiers, all requested Tags and ancestors are revalidated immediately before creation, and this Tool never creates Tags. Before a Project or tagged call, restate the confirmed Project context and full Tag paths so the user can associate the targets with this mutation; if any target is not distinguishable, do not call. Generate a fresh UUID idempotencyKey per new creation intent and reuse it only for a transparent retry. This server may return write_disabled without touching OmniFocus. Parent tasks, repeats, notifications, batches, updates, fallback to Inbox, silent Tag omission, and automatic Tag creation are unsupported.",
    tool: createTaskTool,
    profiles: ["personal-production"],
  },
  {
