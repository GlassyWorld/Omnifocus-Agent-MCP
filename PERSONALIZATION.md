...
- 输出 Active Project summaries、事实型 Attention signals 和 Inbox summaries。
...
Attention 和 Inbox。每个 section 都先完整分类和计数，再稳定排序和截断，并返回
`total`、`returned`、`truncated` 和 `items`。
...
- `planned`：effective Planned 已到达 `generatedAt`，且 Task 当前不是 Blocked。
...
已观察并通过：Active Project、single-actions Project、folder Project、sequential
Project、ordinary Action、Action Group、project-contained Action、planned inherited ready、
planned future、inherited due、direct planned、inherited planned、`hasNote` true 和 false。

未观察到：Inbox Action、Overdue、DueSoon、planned direct ready、Blocked + inherited
planned ready、direct/inherited flag、direct due、direct/inherited defer、multi-reason Task、
Attention + Inbox overlap 和真实 section truncation。`NOT OBSERVED` 不等于失败，没有
为了补齐 Case 修改真实 OmniFocus 数据。

本次 MCP 总耗时约 3527.5 ms；独立 Raw Oracle 中 Task query 约 3430.8 ms，Project
query 约 851.2 ms。该数据只作为 performance evidence，v1 没有性能阈值，也没有据此
修改 Bridge。

Codex client acceptance 结果：PASS。当前会话实际使用 `omnifocus-local`，并确认
`get_lean_snapshot` 已暴露、mutation Tools 仍隐藏。
...
- Inbox Tasks：0。
- 实际观察到的 Attention reason：`planned`。

使用 `limitPerSection: 3` 时，Projects 和 Attention 均稳定返回排序后的前 3 条，
原始总数保持 12 和 8，`truncated` 正确为 true；Inbox 保持 0。非法
`limitPerSection: 0` 稳定返回 `invalid_arguments`。

Client acceptance 同时确认 `scope = "all"`，且没有公开 `raw`、note 全文、Blocked
reason、Waiting、recent completions、health、risk、priority 或 recommendation。
...
```text
Freeze get_lean_snapshot milestone
    |
    v
