...
`get_lean_snapshot` 都是只读 Tool。在开发期间，原始 upstream mutation Tools 可能
仍然注册在 Server 源码中。Codex MCP 使用 `enabled_tools` 作为客户端 allow list，
限制当前客户端配置中实际暴露的 Tools。
...
Snapshot strict adapters
    |
    v
snapshotProjectPlannedResolver + shared Task / Project semantics
    |
    v
...
- 解析 canonical Project root Task 的 Planned semantics。
- 组合 Active Project、Planned-ready Project、Task Attention 和 Inbox 当前事实。
...
- 输出 Active Project summaries、Planned-ready Active Project owners、事实型 Task
  Attention signals 和 Inbox summaries。
...
Planned Projects、Attention 和 Inbox。每个 section 都从完整数据独立分类和计数，再
稳定排序和截断，并返回 `total`、`returned`、`truncated` 和 `items`。
...
即：直接拥有 Planned 的对象，是 Planned 触发可见性的最小单元。表示方式取决于
entity kind：
...
Attention 固定只包含 Task 或 Action Group：

- `overdue`：native `taskStatus === "Overdue"`。
- `dueSoon`：native `taskStatus === "DueSoon"`。
- `planned`：direct Planned 已到达或等于 `generatedAt`，且 Task 当前不是 Blocked。
...
`projects.planned` 只包含 canonical Project root Task 直接拥有且已经到达 Planned
时间的 Active Projects。Project Planned 来源于 canonical root Task semantics，不是
OmniJS Project native Planned property。Project classification 不检查 root Task 是否
Blocked；Project root 本身不会进入 Task Attention。

`projects.planned` 从完整 Active Project summary 集合派生，使用 `planned.direct`、
Project name 和 ID 稳定排序，并与 `projects.active` 独立计数和截断。不得从已经截断的
`projects.active.items` 派生该 section。

`Blocked` 保留为 Task status 和 Project task count，但不是独立 Attention reason，
也不会被解释成 Waiting。v1 不包含 Waiting、recent completions、note 全文、health、
risk、priority、recommendation 或内部 Raw objects。
...
全文；Project 使用 `totalTaskCount`，不读取全部 Task IDs。Project Planned 不增加
Project Raw 字段，而是复用 Task Raw query 中的 `id`、`isProjectRoot`、`plannedDate`
和 `effectivePlannedDate`，通过 canonical Project ID 与 root Task ID 精确 join。
...
- direct Planned owner、inherited Planned no-trigger、四类 Attention reasons、Task
  Blocked 排除和 multi-reason aggregation。
- Project root Planned resolver、missing/duplicate root invariants 和
  `projects.planned` classification。
- Direct/inherited date-source sorting、稳定 tie-breaker 和 section truncation。
- Active Projects、Planned Projects、Inbox、Project root exclusion 和跨 section
  duplication。
- `projects.planned` 从完整 Active Project summaries 独立派生和截断。
- Tool 参数和错误行为。
...
- `completionMapper`
- `attentionClassifier`
- `projectPlannedClassifier`
- `snapshotProjectPlannedResolver`
- `snapshotSorting`
- `leanSnapshotComposer`
- `leanProjectMapper`
...
Planned Project visibility 修正后的真实数据库 server-side acceptance 结果：

- MCP initialize：PASS
- `get_lean_snapshot` registration：PASS
- `generatedAt` call interval validation：PASS
- Raw remaining tasks：155
- Raw Active projects：12
- Planned-ready Active Projects：1
...
核心真实 Case `【重复】每周回顾` 已观察：canonical ID 为 `iS0-YjYC11Y`，root
direct/effective Planned 都是 `2026-07-10T01:00:00.000Z`，并已到达 Snapshot 的
`generatedAt`。该 Project 在 `projects.planned` 中恰好出现一次。其 8 个 children 都
只有 inherited Planned facts，在 Attention 中携带 Planned reason 的 child 数量为 0，
因此 inherited Planned fan-out 已消除。当前数据库时间下 root Task status 为
`DueSoon`；acceptance 使用 Raw Oracle 动态计算其他 Attention reasons 和 totals。

`【重复】每日重置` 也已观察，其 root direct Planned 为
`2026-07-11T01:00:00.000Z`，晚于本次 `generatedAt`，因此状态为
`NOT YET REACHED`。真实数据库没有自然存在的 exact Project Planned boundary，记录为
`NOT OBSERVED`；inclusive boundary 已由 fixture unit test 验证 PASS。

本次 MCP 总耗时约 4184 ms；独立 Raw Oracle 中 Task query 约 2719 ms，Project query
约 968 ms。该数据只作为 performance evidence，没有性能阈值，也没有据此修改 Bridge
或 query architecture。

Planned correction Codex client acceptance 已使用当前会话实际暴露的
`omnifocus-local.get_lean_snapshot` 完成，结果为 PASS。正式采样只调用 Tool 一次，
参数为 `limitPerSection: 100`，并验证 `generatedAt` 位于调用开始和结束时间之间。

Client acceptance 当前事实：

- Active Projects：12，全部返回，未截断。
- Planned-ready Active Projects：2，全部返回，未截断。
- Attention Tasks：8，全部返回，未截断。
- Inbox Tasks：0。
- `attention.byReason`：overdue 0、dueSoon 8、planned 0、flagged 0。
- `【重复】每周回顾` 在 `projects.planned` 中恰好出现一次。
- `【重复】 每日重置` 在 `projects.planned` 中恰好出现一次；其 root Blocked 外部事实
  没有抑制 Project Planned visibility。
- Weekly Review inherited Planned children with Planned reason：0。
- Daily Reset inherited Planned children with Planned reason：0。
- Mutation Tool calls：0。
- OmniFocus writes：0。
- Planned Correction Client Acceptance：PASS。

Client acceptance 同时记录了一个与本次修正相互独立的 Due 事实：Weekly Review 的
8 个 children 都以 `taskStatus = DueSoon`、`due.source = inherited` 和
`reasons = ["dueSoon"]` 出现在 Task Attention 中，ID 集合与外部 Snapshot 完全一致。
因此当前 MCP 的 DueSoon inherited child expansion 已被复现。Due semantics 未在本次
Planned correction 中修改，该事实不影响 Planned Correction Client Acceptance 的
PASS 结论，也不在本里程碑中引入 Due correction 设计。

本次 exact MCP payload 已保存到：

```text
/Users/shixuerui/Desktop/mcp-lean-snapshot-planned-correction-client-acceptance.json
```
...
get_lean_snapshot
    ...
    -> architecture checked
    -> server-side acceptance PASS
    -> planned-project visibility correction implemented
    -> corrected server-side acceptance PASS
    -> corrected Codex client acceptance PASS
```
...
```text
Freeze corrected get_lean_snapshot milestone
    |
    v
