...
> 状态：**已评审接受；Phase 2B 实现与隔离生产 Canary 已通过，正式公开启用仍需独立批准**<br>
> 设计日期：2026-07-14<br>
...
> 上游决策：[ADR-006](../../architecture/decisions/ADR-006-controlled-create-task-v1.md)、[ADR-005](../../architecture/decisions/ADR-005-ai-boundary.md)<br>
> 后续证据：[Phase 2B Project Placement Acceptance](./PHASE2B_PROJECT_PLACEMENT_ACCEPTANCE.md)
...
本设计冻结阶段本身不修改 runtime、不调用 `create_task`、不变更生产 flag、不部署、不重启 Tunnel，也不扩大五 Tool 表面。后续 Phase 2B 实施与隔离 Canary 已按独立授权完成；公开 Project-specific flag 仍为 false。
...
| direct parent | Project 顶层 Task readback 的 `hierarchy.parentId === requestedProjectId`；Project root Task ID 同时是 canonical Project ID | 其他 parent 值停止 Phase 2B；普通 parent Task 仍留在 Phase 4 |
...
actual.hierarchy.parentId === requested projectId
...
OmniFocus 的 Project root Task ID 同时是 mutation 使用的 canonical Project ID。
Project 顶层 action 在 Task read-side 表现为 `project.id` 与 `hierarchy.parentId`
都等于该 root Task ID。这是 Project 顶层 placement，不是 Phase 4 的普通 parent
Task placement；后者的 parent ID 会是另一个普通 Task ID，Phase 2B 必须拒绝。

placement verification 与 destination current eligibility 必须分开：

- 实际 Inbox placement、不同 Project ID、`parentId=null`、其他 Project/普通 Task parent ID、`project.id` 与 `parentId` 不一致，或 Task 属性差异，表示落位/字段没有满足请求，返回 `partial_success`、`mayHaveWritten=true`、`retrySafe=false`；
...
- Task project ID exact，inInbox false，parentId exact 等于 requested Project root Task ID；
...
12. ID readback + Project ID/name + parentId exact 等于 Project root Task ID + Ledger/audit/lock 验证
