...
- Server 通过显式 `personal-readonly` Profile 在 capability registration 层强制只公开四个 Domain read tools；未设置 Profile 时仍进入保留 7 个写入 Tool 的 `upstream-full` 兼容模式。
...
`upstream-full` 当前注册 16 个 Tool；`personal-readonly` 只注册四个 Domain read tools。
...
### 8.1 只读边界依赖显式部署 Profile

`personal-readonly` 已在 Server registration 层排除 generic read tools、mutation tools
和 Resources，不依赖客户端隐藏能力。为保持 backward compatibility，环境变量未设置时
仍进入 `upstream-full`。

风险：部署若忘记显式设置 `OMNIFOCUS_MCP_PROFILE=personal-readonly`，会得到完整兼容
surface。非法值会 fail fast，但未设置是有意的兼容行为。
...
- 后续部署如何确保 ChatGPT Tunnel 始终显式选择 `personal-readonly`？
...
2. 保持 capability boundary：ChatGPT/Tunnel 显式使用 `personal-readonly`，开发与 upstream compatibility 使用 `upstream-full`。
...
> 当前仓库已经形成一个可信的只读 Domain Semantic Layer，并可通过 `personal-readonly`
