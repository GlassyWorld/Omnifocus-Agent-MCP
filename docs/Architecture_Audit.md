...
- Server 通过默认 `personal-production` Profile 在 capability registration 层当前只公开四个 Domain read tools；`upstream-full` 保留 7 个写入 Tool，但只能显式启用。
...
`upstream-full` 当前注册 16 个 Tool；`personal-production` 当前只注册四个 Domain read tools。
...
`personal-production` 已在 Server registration 层排除 generic read tools、mutation tools
和 Resources，不依赖客户端隐藏能力。环境变量未设置或为空时默认进入
`personal-production`；`upstream-full` 只能显式启用。

风险：运维配置若仍使用已移除的旧值 `personal-readonly`，Server 会 fail fast，必须先完成
LaunchAgent Profile 迁移。未来向 `personal-production` 加入能力时，仍需通过显式 profiles
...
- 后续部署如何验证 LaunchAgent 已从旧 Profile 值迁移到 `personal-production`？
...
> Profile 将其四个 Domain read tools 作为独立 Server capability surface 暴露；
