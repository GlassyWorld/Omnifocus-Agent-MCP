...
- Server 通过默认 `personal-production` Profile 在 capability registration 层当前公开四个 Domain read tools 和唯一受控 mutation `create_task`，且不注册 Resources；`upstream-full` 保留原有 7 个写入 Tool，但只能显式启用。
...
`upstream-full` 当前注册 16 个 Tool；`personal-production` 当前精确注册四个 Domain read tools 和 `create_task`。
...
### 8.1 默认 Profile 与显式完整模式边界

`personal-production` 已在 Server registration 层排除 generic read tools、除 `create_task`
外的其他 mutation tools 和 Resources，不依赖客户端隐藏能力。环境变量未设置或为空时默认进入
`personal-production`；`upstream-full` 只能显式启用。

风险：运维配置若仍使用已移除的旧值 `personal-readonly`，Server 会 fail fast。当前部署的
Profile truth 来自 Tunnel Profile YAML，而 `create_task` flag 由主 LaunchAgent plist 承载；
两者不能混淆。未来向 `personal-production` 加入能力时，仍需通过显式 profiles
allowlist 和精确集合测试维护边界。
...
- 后续部署如何持续验证 Tunnel Profile YAML 与主 LaunchAgent plist 各自承担的配置职责没有漂移？
...
2. 保持 capability boundary：ChatGPT/Tunnel 使用默认或显式 `personal-production`，开发与 upstream compatibility 仅显式使用 `upstream-full`。
...
> 当前仓库已经形成一个可信的只读 Domain Semantic Layer，并可通过 `personal-production`
> Profile 将其四个 Domain read tools 与唯一受控 mutation `create_task` 作为精选 Server capability surface 暴露；
> `upstream-full` 继续保留通用查询、Resource 和 mutation 能力。其最成熟的设计不是某个
> 单独实体，而是“严格事实来源 + Task/Project 双视图 + direct-owner Snapshot
> composition”。后续评审应优先保护这一事实边界，而不是立即增加更多 Tool 或推导概念。
