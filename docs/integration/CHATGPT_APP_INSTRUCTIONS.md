...
- For one explicitly requested Inbox task creation, call create_task; no other mutation is available.
...
只公开四个 Domain read tools 和一个受 feature flag 保护的 Inbox create Tool，也不注册 Resources，因此不能沿用完整 Guide 中依赖
...
`personal-production` 表示长期的精选个人生产能力集合。当前版本有四个 Domain read tools、
一个受 feature flag 保护的 `create_task` 且无 Resources。代码默认 fail closed；当前生产
...
- 用户请求：明确创建一个不含 Project/parent/Tag 等扩展要求的 Inbox Task。
- 应调用的 Tool：`create_task`；客户端确认后调用。
