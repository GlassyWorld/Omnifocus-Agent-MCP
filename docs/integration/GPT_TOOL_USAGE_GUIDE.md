...
6. `create_task` V3（仅显式 Inbox 或 exact Active Project 创建；可选 1–5 个既有 eligible Tag canonical IDs；代码默认仍 fail closed）

五个 read tools 应优先于 upstream generic interfaces。`create_task` 不参与分析，只处理用户明确授权的单个 Inbox 或 exact Active Project 创建请求；tagged intent 必须先经 fresh `search_tags`、完整 path 确认并只提交 canonical IDs。Parent 等其他要求仍不支持。
...
`personal-production` 是已实现的 Server-side curated capability boundary。`create_task` V3 只允许
显式创建一个 Inbox Task 或放入一个 freshly-read exact Active Project，并可选添加 1–5 个
...
Ledger/JXA；其他 mutation 仍不可发现。分析或建议不构成 mutation authorization。Tag 不按名称
解析、不自动创建；不能完整满足 Tag 要求时不得省略 `tagIds` 或静默创建无 Tag Task。
