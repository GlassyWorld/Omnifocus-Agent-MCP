...
`personal-production` 当前注册集合包含四个既有 Domain reads、只读 `search_tags` 和 ADR-006 的 `create_task` V2；后者
在代码默认值下由 `OMNIFOCUS_CREATE_TASK_ENABLED=false` 于 Ledger/JXA 前禁写，当前生产
LaunchAgent 则经 Checkpoint 7 验收后显式设置为 `true`。该限制是 Server-side enforcement，
不是客户端 allowlist。
...
6. `create_task` V2（仅显式 Inbox 或 exact Active Project 创建；代码默认仍 fail closed）

五个 read tools 应优先于 upstream generic interfaces。`create_task` 不参与分析，只处理用户明确授权、且不含 Tag/parent 等未支持要求的单个 Inbox 或 exact Active Project 创建请求。
...
`personal-production` 是已实现的 Server-side curated capability boundary。`create_task` V2 只允许
显式创建一个 Inbox Task 或放入一个 freshly-read exact Active Project；每个新意图使用新的 UUID `idempotencyKey`，透明 retry 复用同一 key。
flag 关闭时返回 `write_disabled` 且不得进入 Ledger/JXA；其他 mutation 仍不可发现。分析或建议
不构成 mutation authorization。V2 不支持 Tag assignment；包含 Tag 要求的创建请求不得调用
`create_task`，更不得静默创建无 Tag Task。
...
用户明确要求创建一个无 unsupported fields 的 Inbox / exact Active Project Task
    -> create_task

用户要求创建 Task 并添加 Tag
    -> 不调用 create_task；说明 Tag assignment 尚未授权
