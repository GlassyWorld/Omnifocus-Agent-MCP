...
`personal-production` 当前注册集合只包含四个 Domain reads 和 ADR-006 的 `create_task`；后者
...
5. `create_task`（仅显式 Inbox 创建；当前生产已启用，代码默认仍 fail closed）

四个 Domain read tools 应优先于 upstream generic interfaces。`create_task` 不参与分析，只处理用户明确授权的单个 Inbox 创建请求。
...
`upstream-full` 实际注册 16 个 Tool。`personal-production` 当前注册四个 Personalized
Domain Read Tools 和一个受 feature flag 保护的 `create_task`，并且不注册 Resources。
...
`personal-production` 是已实现的 Server-side curated capability boundary。`create_task` 只允许
显式创建一个 Inbox Task；每个新意图使用新的 UUID `idempotencyKey`，透明 retry 复用同一 key。
flag 关闭时返回 `write_disabled` 且不得进入 Ledger/JXA；其他 mutation 仍不可发现。分析或建议
不构成 mutation authorization。
