...
`personal-production` 当前注册集合中，这些能力不会被 Server 注册。该限制是 Server-side
enforcement，不是客户端 allowlist。该 Profile 表示长期精选生产能力集合，并非永久只读；
未来写入必须通过新的 Accepted ADR 和专门受控 Tool 显式加入。
...
`upstream-full` 实际注册 16 个 Tool。`personal-production` 当前只注册四个 Personalized
...
`personal-production` 是已实现的 Server-side curated capability boundary，当前注册集合只读。
本指南仍不定义 write workflow；完整模式保留 upstream compatibility，但分析或建议不构成
