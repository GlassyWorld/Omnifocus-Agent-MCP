...
Instructions 是面向 `personal-production` Profile 的压缩执行版本。两者出现语义冲突时，
...
未设置或设置为空时安全默认进入 `personal-production`；`upstream-full` 只能显式启用。
...
的 `create_task` V4；后者由 global/Project/Tag/Parent 四层 flag fail closed。V4 build、ChatGPT App
Refresh、禁写客户端门禁、Parent Canary 与正式启用已通过；当前 loaded
global=true、Project=true、Tag=true、Parent=true。该限制是 Server-side enforcement，
不是客户端 allowlist。
...
5. `search_tags`（只读发现既有 Tag；默认 Active；完整 path 消歧）
6. `create_task` V4（仅显式 Inbox、exact Active Project 或 freshly-read exact eligible ordinary Action Group 创建；可选 1–5 个既有 eligible Tag canonical IDs；代码默认仍 fail closed）

五个 read tools 应优先于 upstream generic interfaces。`create_task` 不参与分析，只处理用户明确授权的单个 Inbox、exact Active Project 或 eligible ordinary Action Group 创建请求；Project 与 Parent 目标必须先经 fresh read 绑定 canonical ID，tagged intent 必须先经 fresh `search_tags`、完整 path 确认并只提交 canonical IDs。Leaf Action parent、Project Root parent、repeat、notification、batch、edit、move/reparent、complete、delete 等其他要求仍不支持。
...
`upstream-full` 实际注册 16 个 Tool。`personal-production` 当前注册五个 read Tool（四个
Domain reads + `search_tags`）和一个受 feature flag 保护的 `create_task`，并且不注册 Resources。
...
| `search_tags` | 既有 Tag 的结构化只读发现 | canonical ID、三态、完整 path、同名消歧、互斥事实 | Tag 创建/修改、Task Tag assignment、按 Tag 查询 Task | 通常无需；只为当前 Tag 问题缩小 query |

这些 Tool 返回的是 AI-facing Domain View，不是简单 Raw query。

#### Structured output contract

五个 personal-production read Tool 均在 MCP descriptor 中声明 `outputSchema`。成功结果通过
...
#### `upstream-full` 实际注册的 generic read tools
...
这些 Tool 只在 `upstream-full` 中注册；`personal-production` 不暴露 generic read tools 或 Resources。显式选择 `upstream-full` 时，它们用于补充 Domain Tool 无法表达的查询：
...
Resource 可以作为补充上下文，但不能视为 `personal-production` 五个 read Tool 的替代接口。
...
- 只在 `upstream-full` 注册，在当前 `personal-production` 中不可发现、不可调用。
- 不属于默认分析流程。
- 不得因为 Snapshot、Attention、推断或建议而自动触发。
- 只有用户明确提出具体写入请求时，才可视为新的 mutation request，进入独立的授权与确认流程。

`personal-production` 是已实现的 Server-side curated capability boundary。`create_task` V4 只允许
显式创建一个 Inbox Task、放入一个 freshly-read exact Active Project，或创建到一个
freshly-read exact eligible ordinary Action Group，并可选添加 1–5 个
freshly-discovered、ancestor-active、非互斥的既有 Tag canonical IDs；每个新意图使用新的 UUID
`idempotencyKey`，透明 retry 复用同一 key。任一适用 flag 关闭时返回 `write_disabled` 且不得进入
Ledger/JXA；其他 mutation 仍不可发现。分析或建议不构成 mutation authorization。Project、Parent
和 Tag 不按名称或 path 解析，不自动创建；不能完整满足目标或 Tag 要求时不得 fallback 到 Inbox /
Project、省略 `tagIds` 或静默创建无 Tag Task。
...
用户询问已有 Tag 或需要同名 path 消歧
    -> search_tags

用户明确要求创建一个无 unsupported fields 的 Inbox / exact Active Project / exact eligible ordinary Action Group Task
    -> create_task

用户要求创建 Task 并添加既有 Active Tag
    -> search_tags
    -> 确认完整 path 与 canonical ID 后 create_task

用户要求 Domain Tool 无法表达的特定筛选
    -> personal-production 中澄清限制或请求导出/切换授权
    -> upstream-full 明确启用时可用 query_omnifocus
...
flowchart TD
    ...
    HISTORY -->|否| FILTER
    FILTER -->|是| GQ["personal-production: 澄清限制<br/>upstream-full: query_omnifocus"]
    FILTER -->|否| AUDIT
...
Accepted ADR-004 明确：Full Snapshot 不要求成为 MCP Tool。当前低频深度审计继续使用插件或文件导出；只有真实、重复的个人需求出现后才重新评估开发。上图中的 `query_omnifocus` 只适用于显式启用的 `upstream-full`，不属于 `personal-production` 当前 surface。
...
### `query_omnifocus`（仅 `upstream-full`）

仅在以下情形使用：

- `personal-production` 五个 read Tool 无法表达、且用户已明确进入 `upstream-full` 或等价授权上下文的特定过滤。
...
- 不任意选择。
- 在 `personal-production` 中，使用已返回的上下文请求用户补充准确名称、ID 或区分信息。
- 只有显式进入 `upstream-full` 或等价授权上下文时，才可使用 `query_omnifocus` 进行只读候选查询，返回最少必要字段，如 ID、name、Project/Folder context。
- 将候选项交给用户确认后，再使用 ID 调用 Domain Tool。

### 对象不存在

`not_found` 表示没有 exact match。不要猜测 ID，也不要把 partial name 自动当成目标对象。可以询问用户；只有显式进入 `upstream-full` 或等价授权上下文时，才使用通用只读查询缩小范围。
...
- 优先请求 ID 或更完整名称。
- 只有显式进入 `upstream-full` 或等价授权上下文时，才使用 `query_omnifocus` 做只读候选发现。
...
- `personal-production`：当前没有按 Tag 查询 Task 集合的 Tool；可先用 `search_tags` 确认 Tag identity，再说明该集合筛选需要导出、文件输入或显式 `upstream-full` 上下文。
- `upstream-full` 首选 Tool：`query_omnifocus`，因为当前个人生产 read surface 不提供 Tag 集合筛选。
...
- 通用筛选问题只有在显式 `upstream-full` 或等价授权上下文中才使用 `query_omnifocus`。
...
2. Domain Tool 的 Adapter/Contract failure 与 OmniFocus/bridge execution failure 最终都
   ...
   消息区分。
3. `upstream-full` 仍注册 7 个 mutation tools；`personal-production` 当前在 Server registration
