...
| `personal-production` 是 server-side curated capability boundary，当前为四读一写 | profile/registration 代码与测试、[PROJECT_STATUS](./PROJECT_STATUS.md) | README、Audit、Tunnel 手册 | 旧 `personal-readonly` 名称；仅依赖客户端 allowlist/提示词形成安全边界 | `src/config/serverProfile.ts`、`src/serverRegistration.ts` 及 tests |
...
| `personal-production` 当前精确注册四个 Domain read tools、`create_task` 且无 Resources | 当前代码/测试、[状态页](./design/personal-production/README.md) | Profile refactor 与 create_task 验收记录 | 从 `create_task` 推断其他 mutation 能力 | registration allowlist、精确五 Tool 集合和 Resource tests |
