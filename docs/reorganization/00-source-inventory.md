...
| D. 冻结设计与验收记录 | 2 | `README.md`、`PERSONALIZATION.md` 同时承担当前冻结基线说明 |
...
|---|---|---|---|---|---|---|---|---|---|
| `README.md` | 冻结设计与项目入口 | 四个 Domain Tool、日期语义、Profile、安全边界 | 2025-03 起；2026-07-12 当前基线 | 当前 | 冻结契约/项目入口 | 与 `PERSONALIZATION.md`、两份架构审计重叠 | Roadmap 未包含用户后续提出的 `personal-production`/`create_task`，但不构成代码冲突 | KEEP；由 `docs/README.md` 导航 | 标题标记 `v1.0-personalized`；列出四个工具、Profile 与维护来源 |
| `PERSONALIZATION.md` | 冻结设计与验收记录 | Domain 模型、四工具、验收快照、未来复审边界 | 2026-07-10 至 2026-07-12 | 当前；其中数量是历史验收快照 | 冻结契约/工程汇总 | 与 README、工程日志和 ADR 大量重叠 | 早期 Snapshot 过程已由文内后续修订收束 | KEEP；索引，不拆分 | 末尾明确 Full Snapshot、Action、mutation gateway 边界及“当前没有承诺的新 Tool” |
| `QUERY_TOOL_REFERENCE.md` | 用户使用文档 | `query_omnifocus` 参数、字段、过滤和输出 | 2025-08 起；2026-07-11 校准 | 当前 | 实现参考 | 与 Examples、GPT Guide 的 generic read 章节重叠 | `query_omnifocus` 不在 `personal-readonly`；文档自身未声称属于该 Profile | KEEP | 2026-07-11 提交按当前代码更新日期过滤与输出说明 |
...
| `docs/Architecture_Audit.md` | 用户/架构文档 | 当前仓库结构、数据流、Domain、工具、风险、演进 | 2026-07-11；2026-07-12 更新 | 当前 | 最新综合架构审计 | 与 `_v1`、README、PERSONALIZATION 重叠 | 第 8.10 节“Owner entity”是审计判断，不是已接受独立 Domain | KEEP；设为详细审计权威入口 | 含 16/4 Tool surface、结构化输出和最新 Profile 事实 |
...
| `docs/OmniFocus-MCP-Tunnel日常维护与新增Tool操作手册.md` | 用户/运维文档 | Tunnel、LaunchAgent、Profile、Tool 发布流程 | 2026-07-12 | 当前；机器环境细节需部署时复核 | 运维说明 | 与 GPT Guide/Profile 文档重叠 | 全文以 `personal-readonly` 为当前生产 Profile；后续重构后将需同步更新 | KEEP；列入 operations 导航 | 开头声明 macOS + launchd + Tunnel + personal-readonly 基线 |
...
| `docs/architecture/decisions/ADR-003-task-action-boundary.md` | ADR | Action 留在 Task Domain | 2026-07-11 | 当前 | Accepted ADR | 与审计、PERSONALIZATION 重叠 | 取代立即新增 `ActionView`/`get_action` 的讨论方向 | KEEP | 状态 Accepted；列出未来复审条件 |
| `docs/architecture/decisions/ADR-004-lean-snapshot-scope.md` | ADR | Lean/Full Snapshot 边界、独立截断 | 2026-07-11 | 当前 | Accepted ADR | 与 Snapshot 工程日志、PERSONALIZATION 重叠 | 取代立即开发 Full Snapshot MCP 的旧倾向 | KEEP | 状态 Accepted；明确低频需求采用手动/plugin/file 导出 |
...
| `docs/integration/CHATGPT_APP_INSTRUCTIONS.md` | 运行时指令 | 四工具路由、事实/推断边界、写入拒绝 | 2026-07-12 | 当前 | 生产运行指令；低于代码/ADR | GPT Guide 的压缩版本 | 只适用于 `personal-readonly`，未来 Profile 变更需更新 | KEEP | Design Notes 明确它也是 Server Instructions 规范内容且能力边界由注册表实现 |
...
| `engineer_log/GET_TASK_ENGINEERING_LOG.md` | 工程执行日志 | `get_task` 设计、实现、验收 | 2026-07-10/11 | 当前历史 | 冻结工程记录 | 与 PERSONALIZATION Task 章节、ADR-003 重叠 | 无 | KEEP；加入 engineer_log 索引 | 记录 Unit、server-side、Codex client acceptance |
| `engineer_log/GET_PROJECT_ENGINEERING_LOG.md` | 工程执行日志 | `get_project` 设计、实现、验收 | 2026-07-10/11 | 当前历史 | 冻结工程记录 | 与 PERSONALIZATION Project 章节、ADR-002 重叠 | 无 | KEEP；加入索引 | 记录 canonical Project ID 和验收结果 |
| `engineer_log/GET_COMPLETED_SINCE_ENGINEERING_LOG.md` | 工程执行日志 | completion event 流、实现、验收 | 2026-07-10/11 | 当前历史 | 冻结工程记录 | 与 PERSONALIZATION Completion 章节重叠 | 无 | KEEP；加入索引 | 明确 direct `completionDate` 和排除 project root event |
